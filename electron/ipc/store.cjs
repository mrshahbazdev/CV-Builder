...
/**
 * Document store. The whole CV is one JSON document written atomically
 * (tmp file + rename), plus timestamped snapshots in history/ so an
 * autosave or a bad edit can never destroy work.
 *
 * Chosen over SQLite because the document is a single nested tree — there
 * are no queries to optimise, and a plain file keeps a native dependency
 * out of the packaged installer.
 */
...
function docPath() {
  return path.join(storeDir(), 'cv.json');
}
