...
// A CV holds addresses, phone numbers and employer names — it should never
// be able to reach the network from the renderer, full stop.
function applyCSP() {
  const policy = isDev
...
function registerAppIPC() {
  ...
  ipcMain.handle('app:openFile', async (_e, { filters } = {}) => {
    ...
    if (['.pdf', '.docx'].includes(ext)) {
      return { path: filePath, name: path.basename(filePath), base64: fs.readFileSync(filePath).toString('base64') };
    }
